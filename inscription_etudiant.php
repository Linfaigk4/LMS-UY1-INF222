<?php
/**
 * GOL (Gugle Online Learning) - Inscription Étudiant
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nom) || empty($prenom) || empty($email) || empty($password)) {
        ...
        $resultat = inscrireUtilisateur($email, $password, $nom, $prenom, 'etudiant');
        
        if ($resultat['success']) {
            // Enregistrer le téléphone si fourni
            if (!empty($telephone)) {
                $pdo = connexionBDD();
                $stmt = $pdo->prepare("UPDATE utilisateurs SET telephone = ? WHERE id_utilisateur = ?");
                $stmt->execute([$telephone, $resultat['id']]);
            }
...
$page_title = 'Inscription Étudiant - GOL';
?>
...
.inscription-header {
    ...
    text-align: center;
    background: linear-gradient(135deg, var(--primaire-sombre) 0%, var(--secondaire) 100%);
    color: white;
}
        ...
        <div class="inscription-form">
            ...
            <form method="POST" action="" id="inscriptionForm">
                ...
                <div class="input-group">
                    ...
                    <input type="tel" name="telephone" class="input-field" value="<?= htmlspecialchars($_POST['telephone'] ?? '') ?>" placeholder="Optionnel">
                </div>
